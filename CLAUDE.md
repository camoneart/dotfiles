...
ユーザーからアプリ開発の指示があったとき、具体的な技術指示が無ければ、"Next.js", "TypeScript", "shadcn/ui"をベースにして開発するように。

### カラー
ユーザーから使用カラーの指定が無ければ、背景：暗い, テキスト：明るい を基準としたカラーにする。

### スタイル
ユーザーからスタイルの指定がなければ、"シンプル", "シック", "モダン", "黒白" をベースとしたスタイルを組むように。
